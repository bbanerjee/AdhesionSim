%
% Create an Abaqus input file for two surfaces in close proximity
%
function createAbaqusInput(n, refineLevel)
  ...
  %
  % Create the mesh for the bottom part
  %
  bot = -1;
  startNode = 0;
  startElem = 0;
  ...
  %
  % Create the mesh for the top part
  %
  top = 1;
  %startNode = len(nodeBot);
  %startElem = len(elemBot);
  startNode = 0;
  startElem = 0;
  gridspacing = 1.0;
  [nodeTop, elemTop] = createMesh(n, gridsize, gridspacing, top, startNode, startElem, ...
                                  refineLevel);
  plotMesh(nodeTop, elemTop);
  ...
  %
  % Select boundary condition nodes for top part
  %
  [nodeFixedTop, nodeXsymmTop, nodeYsymmTop] = selectNodes(nodeTop, top);
  ...
  %
  % Select contact elements for the top part (slave)
  %
  [contactElemTop] = selectElems(elemTop, top);

  %
  % Select contact nodes for the top part
  %
  [contactNodeTop] = selectContactNodes(nodeTop);
  ...
  %
  fid = fopen('AdhesionSurf.inp','w');
  writeAbaqusHeader(fid);
  writeAbaqusMesh(fid, nodeBot, elemBot, nodeTop, elemTop);
  ...
  fprintf(fid, '*Instance, name=TopBlock-1, part=TopBlock\n');
  fprintf(fid, '*End Instance\n');
  fprintf(fid, '**\n');
  ...
  fprintf(fid, '*Nset, nset=nodeFixedTop, internal, instance=TopBlock-1\n');
  writeAbaqusNodeSet(fid, nodeFixedTop);
  fprintf(fid, '*Nset, nset=nodeXsymmTop, internal, instance=TopBlock-1\n');
  writeAbaqusNodeSet(fid, nodeXsymmTop);
  fprintf(fid, '*Nset, nset=nodeYsymmTop, internal, instance=TopBlock-1\n');
  writeAbaqusNodeSet(fid, nodeYsymmTop);
  fprintf(fid, '*Nset, nset=contactNodeBot, internal, instance=BottomBlock-1\n');
  writeAbaqusNodeSet(fid, contactNodeBot);
  fprintf(fid, '*Nset, nset=contactNodeTop, internal, instance=TopBlock-1\n');
  writeAbaqusNodeSet(fid, contactNodeTop);
  writeAbaqusNodeSurface(fid, contactNodeBot, 'contactNodeBot', bot);
  writeAbaqusNodeSurface(fid, contactNodeTop, 'contactNodeTop', top);
  writeAbaqusElemSurface(fid, contactElemBot, 'BottomBlock-1', bot);
  writeAbaqusElemSurface(fid, contactElemTop, 'TopBlock-1', top);
...
function writeAbaqusMesh(fid, nodeBot, elemBot, nodeTop, elemTop)
  ...
  % Top part next
  %
  fprintf(fid, '*Part, name=TopBlock\n');
  fprintf(fid, '*Node\n');
  numNodeTop = size(nodeTop(:,1));
  for ii=1:numNodeTop
    fprintf(fid, '%5d, %16.5f, %16.5f, %16.5f\n', ...
      ii, nodeTop(ii,2), nodeTop(ii,3), nodeTop(ii,4));
  end
  fprintf(fid, '*Element, type=C3D8RH\n');
  numElemTop = size(elemTop(:,1));
  for ii=1:numElemTop
    fprintf(fid, '%5d, %5d, %5d, %5d, %5d, %5d, %5d, %5d, %5d\n', ...
      ii, elemTop(ii,2), elemTop(ii,3), elemTop(ii,4), elemTop(ii,5), ...
      elemTop(ii,6), elemTop(ii,7), elemTop(ii,8), elemTop(ii,9));
  end
  fprintf(fid, '*Nset, nset=TopMesh, internal, generate\n');
  fprintf(fid, '    1, %5d,    1\n', numNodeTop);
  fprintf(fid, '*Elset, elset=TopMesh, internal, generate\n');
  fprintf(fid, '    1, %5d,    1\n', numElemTop);
  fprintf(fid, '** Section: TopSection\n');
  fprintf(fid, '*Solid Section, elset=TopMesh, material="Elastomer"\n');
  fprintf(fid, ',\n');
  fprintf(fid, '*End Part\n');
...
function writeAbaqusElemSurface(fid, elemSet, name, loc)
  ...
  if (loc == -1)
    ...
    fprintf(fid, '*Surface, type=ELEMENT, name=BottomContactSurf, internal\n');
    ...
    fprintf(fid, '*Surface, type=ELEMENT, name=TopContactSurf, internal\n');
...
function writeAbaqusInteraction(fid)
  ...
  fprintf(fid, 'TopContactSurf, BottomContactSurf\n');
  %fprintf(fid, '*Contact Pair, interaction=Contact-1, supplementary constraints=YES\n');
  %fprintf(fid, 'contactNodeTop, BottomContactSurf\n');
  %fprintf(fid, 'contactNodeBot, TopContactSurf\n');
...
function writeAbaqusStep(fid)
  ...
  fprintf(fid, 'NodeFixedTop, 3, 3, -2.5\n');
  fprintf(fid, 'NodeXSymmTop, 1, 1, 0.0\n');
  fprintf(fid, 'NodeYSymmTop, 2, 2, 0.0\n');
...
function [node, elem] = createMesh(n, gridsize, gridspacing, loc, startNode, startElem, ...
  ...
  % Create the surface
  %
  [x, y, z, gridsize] = createSurf(n, gridsize, gridspacing, refineLevel);
...
%  Create surface 
%
function [x, y, surfelev, gridsize] = createSurf(n, gridsize, gridspacing, refineLevel)
