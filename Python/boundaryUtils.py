...
#-------------------------------------------------------------
# Get the face (sequence of faces) that contains a given point
#
def getFace(cntnr, pt ):

  face = cntnr.faces[0:0]
  for i in range(len(cntnr.faces)):
    face += cntnr.faces.findAt((pt,), printWarning=False)
    return (face)
  return(0)

#-------------------------------------------------------------
# Get the cell (sequence of cells) that contains a given point
#
def getCell(cntnr, pt ):

  cell = cntnr.cells[0:0]
  for i in range(len(cntnr.cells)):
    cell += cntnr.cells.findAt((pt,), printWarning=False)
    return (cell)
  return(0)
